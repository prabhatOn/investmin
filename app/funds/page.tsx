...
export default function FundsPage() {
  ...
  // Force mobile bottom nav (full-width content) for mid-size viewports (tablet)
    ...
    return (
      <div className="trading-dashboard min-h-screen flex flex-col relative">
        ...
        <div className="flex flex-1 overflow-hidden relative z-10">
          <TradingSidebar collapsed={sidebarCollapsed} onCollapsedChange={setSidebarCollapsed} forceMobileNav={forceMobileOnTablet} />
          <main className={`flex-1 flex items-center justify-center transition-all duration-300 ${
            forceMobileOnTablet ? "pl-4 pr-6" : (sidebarCollapsed ? "sm:pl-20 pl-4 pr-6" : "sm:pl-68 pl-4 pr-6")
          }`}>
  ...
  if (!activeAccount) {
    return (
      <div className="trading-dashboard mb-5 min-h-screen flex flex-col relative">
        {/* Enhanced pattern overlay for more visible texture */}
        <div 
          className="absolute inset-0 opacity-20 pointer-events-none z-0"
          style={{
  ...
  return (
    <div className="trading-dashboard sm:mb-10 md:mb-10 min-h-screen flex flex-col relative">
      {/* Enhanced pattern overlay for more visible texture */}
      <div 
        className="absolute inset-0 opacity-20 pointer-events-none z-0"
        style={{
      ...
      <div className="flex flex-1 overflow-hidden relative z-10">
        ...
        <main className={`flex-1 flex flex-col gap-6 overflow-auto transition-all duration-300 w-full max-w-screen-xl mx-auto ${
          forceMobileOnTablet ? "pl-4 pr-4 sm:pr-6 pt-6 sm:pb-6 pb-28" : (sidebarCollapsed ? "sm:pl-20 pl-4 pr-4 sm:pr-6 pt-6 sm:pb-6 pb-28" : "sm:pl-68 pl-4 pr-4 sm:pr-6 pt-6 sm:pb-6 pb-28")
        }`}>
          {/* Page Header */}
                ...
                <CardContent className="flex flex-col items-center px-4 py-2">
                  <div className="space-y-3 w-full">
                    {pendingDeposits.map((d) => (
                      <div
                        key={d.id}
                        className="w-full bg-background p-3 rounded-md flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 min-w-0"
                      >
                        <div className="min-w-0 flex-1 overflow-hidden">
                          <div className="font-medium text-sm whitespace-normal break-all">Request: <span className="whitespace-normal break-all">{d.transaction_id}</span></div>
                          <div className="text-xs text-muted-foreground whitespace-normal break-words mt-1">Amount: ${Number(d.amount).toFixed(2)} • Fee: ${Number(d.fee || 0).toFixed(2)}</div>
                        </div>

                        <div className="text-sm sm:ml-4 sm:flex-shrink-0 w-full sm:w-auto mt-2 sm:mt-0">
